...
def create_app(test_config=None):
  ...
  '''
    ...
    def get_categories():
        ...
        # used categories in response
        # -> TypeError: Object of type Category is not JSON serializable
        # have to format categories!!
        # formatted_categories = [category.format() for category in categories]
        # -> works in list but in 'add' got error
  ...
  '''
    def paginate_questions(request, selection):
        # use request to be able to use its arguments to get the page number or
        # 1 if no page number
